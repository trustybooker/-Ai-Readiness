...
function publicBase(){ return (process.env.PUBLIC_SITE_URL || 'https://ai-readiness-pass.netlify.app').replace(/\/$/, ''); }
...
export default async (req) => {
  ...
  const caller=safeCaller(params.From); const contact=callContact(params,caller);
  ...
  const message=callerMessage(params); const isNewCall=!message;
  if(isNewCall)return twiml(gather('Thank you for calling AI Kollege, operated by Fify Now. I am the AI Secretary. You can speak naturally. I can explain our AI readiness score, training paths, courses, badge, and booking process. For a person, say human or press zero. How may I help you?')+'<Say>I did not hear anything. Please call again when you are ready.</Say><Hangup/>');
  if(/\b(human|person|representative|agent|operator)\b/i.test(message)||params.Digits==='0'){
    await logConversationLead({result:{reply:'Caller requested human follow-up.',handoff:true,lead:true,offer_booking:true},message,history:[],channel:`phone:${caller}`,contact}).catch(()=>null);
  ...
  try{
    ...
    if(result.handoff){ const transfer=String(process.env.TWILIO_HUMAN_FORWARD_NUMBER||'').trim(); if(transfer)return twiml(`<Say>${esc(reply)}</Say><Say>I will try to connect you with a person now.</Say><Dial timeout="20" answerOnBridge="true">${esc(transfer)}</Dial><Say>No one was available. Your request has been marked for human follow-up.</Say><Hangup/>`); return twiml(`<Say>${esc(reply)}</Say><Say>Your request has been marked for human follow-up.</Say><Hangup/>`); }
  ...
  }catch{return twiml('<Say>I am having trouble answering right now. Please use the booking or request form on the AI Kollege website and a human will follow up. Goodbye.</Say><Hangup/>');}
};
