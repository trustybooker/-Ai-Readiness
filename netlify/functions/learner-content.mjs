...
export default async(req)=>{
  ...
  const session=await r.json();const c=classifyCheckoutSession(session);if(!c.ok)return json(403,{ok:false,error:'paid_entitlement_not_verified'});
  const payload=learnerPayload(c.offer.key,profile);if(!payload)return json(403,{ok:false,error:'unsupported_offer'});
  return json(200,{ok:true,verified:true,...payload});
};
