...
export const SOCIAL_CONTROL_VERSION='2026-08-15-v1';
...
export function platformReadiness(env=process.env){return {
 ...
 tiktok:{configured:Boolean(env.TIKTOK_ACCESS_TOKEN&&env.TIKTOK_OPEN_ID),account:clean(env.TIKTOK_OPEN_ID,180),publish_capability:'content-posting-adapter-required'}
};}
export function socialCapabilities(env=process.env){return {version:SOCIAL_CONTROL_VERSION,platforms:[...SOCIAL_PLATFORMS],publish_enabled:String(env.SOCIAL_PUBLISH_ENABLED||'').toLowerCase()==='true',approval_authority:['owner'],proposal_authority:['owner','momo'],customer_systems:['secretary','receptionist'],customer_system_marketing_publish:false,destructive_remote_actions:false,readiness:platformReadiness(env)};}
...
export function assertActorPermission(actor,action){
 ...
 if(['approve','schedule','publish','cancel'].includes(action)&&actor!=='owner')throw new Error('owner_approval_required');
 return true;
}
...
export function publicationGate(op,env=process.env){if(op?.state!=='approved'&&op?.state!=='scheduled')return{ok:false,error:'not_approved'};if(!op.approval||op.approval.fingerprint!==op.fingerprint)return{ok:false,error:'stale_approval'};if(String(env.SOCIAL_PUBLISH_ENABLED||'').toLowerCase()!=='true')return{ok:false,error:'publishing_disabled'};const ready=platformReadiness(env),missing=op.platforms.filter(p=>!ready[p]?.configured);if(missing.length)return{ok:false,error:'platform_not_configured',platforms:missing};return{ok:true,readiness:ready};}
