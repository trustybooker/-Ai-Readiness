...
export const COURSE_KEYS=['ai_starter_pass','ai_job_productivity_pass'];
const COURSE_MODULES={ai_starter_pass:new Set(['boundaries','prompt','verify','privacy','approval','proof']),ai_job_productivity_pass:new Set(['resume','interview','rolepack','workflow','case','signal'])};
const E164=/^\+[1-9]\d{7,14}$/;
const HHMM=/^(?:[01]\d|2[0-3]):[0-5]\d$/;
const UNSAFE_COURSE_CLAIM=/\b(?:accredited|accreditation|professional license|licensed credential|official certification|guaranteed (?:job|employment|income|revenue)|regulatory certification)\b/i;
...
function safeCourseText(value,max=1200){const text=String(value??'').replace(/[\u0000-\u001f\u007f<>]/g,' ').replace(/\s{3,}/g,' ').trim().slice(0,max);return UNSAFE_COURSE_CLAIM.test(text)?'':text;}
function defaultCourseOperations(){return{ai_starter_pass:{selfServeEnabled:true,learnerNotice:'',supplements:{}},ai_job_productivity_pass:{selfServeEnabled:true,learnerNotice:'',supplements:{}}};}
...
export function sanitizeOwnerSettings(input={},base=defaultOwnerSettings()){const out=structuredClone(base);out.version=2;if(typeof input.secretaryEnabled==='boolean')out.secretaryEnabled=input.secretaryEnabled;if(typeof input.webSecretaryEnabled==='boolean')out.webSecretaryEnabled=input.webSecretaryEnabled;if(typeof input.whatsappCustomerEnabled==='boolean')out.whatsappCustomerEnabled=input.whatsappCustomerEnabled;if(ALLOWED_TONES.includes(input.customerTone))out.customerTone=input.customerTone;if(ALLOWED_AUTONOMY.includes(input.assistantAutonomy))out.assistantAutonomy=input.assistantAutonomy;const booking=httpsUrl(input.bookingUrl);if(booking)out.bookingUrl=booking;
  ...
  if(input.courseOperations&&typeof input.courseOperations==='object'){
    out.courseOperations=out.courseOperations||defaultCourseOperations();
    for(const courseKey of COURSE_KEYS){const incoming=input.courseOperations[courseKey];if(!incoming||typeof incoming!=='object')continue;const target=out.courseOperations[courseKey]||defaultCourseOperations()[courseKey];if(typeof incoming.selfServeEnabled==='boolean')target.selfServeEnabled=incoming.selfServeEnabled;if(Object.hasOwn(incoming,'learnerNotice'))target.learnerNotice=safeCourseText(incoming.learnerNotice,500);if(incoming.supplements&&typeof incoming.supplements==='object'){const next={...target.supplements};for(const [moduleId,value] of Object.entries(incoming.supplements)){if(!COURSE_MODULES[courseKey].has(moduleId))continue;const text=safeCourseText(value,1200);if(text)next[moduleId]=text;else delete next[moduleId];}target.supplements=next;}out.courseOperations[courseKey]=target;}
  }
  return out;
}
...
export async function privateStoreGate(){const repo=String(process.env.LEADS_REPO||''),secret=String(process.env.LEADS_SECRET||'');if(!secret||!repo.includes('/'))return{ok:false,error:'leads_not_configured'};const r=await fetch(`https://api.github.com/repos/${repo}`,{headers:headers()});if(!r.ok)return{ok:false,error:`github_${r.status}`};const meta=await r.json();return meta?.private===true?{ok:true,repo}:{ok:false,error:'unsafe_lead_repository'};}
