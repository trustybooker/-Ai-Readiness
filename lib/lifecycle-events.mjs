...
import {verifyCheckoutEntitlement} from './stripe-fulfillment.mjs';
...
async function verifySession(sessionId){const id=clean(sessionId,160);if(!SESSION_RE.test(id))return{ok:false,error:'invalid_session_id'};const key=String(process.env.STRIPE_SECRET_KEY||'');if(!key)return{ok:false,error:'stripe_verification_not_configured'};const r=await fetch(`https://api.stripe.com/v1/checkout/sessions/${encodeURIComponent(id)}`,{headers:{Authorization:`Bearer ${key}`}});if(r.status===404)return{ok:false,error:'session_not_found'};if(!r.ok)return{ok:false,error:'stripe_lookup_failed'};const entitlement=await verifyCheckoutEntitlement(await r.json(),key);return entitlement.ok?{ok:true,purchase:entitlement}:{ok:false,error:entitlement.error||'paid_entitlement_not_verified'};}
