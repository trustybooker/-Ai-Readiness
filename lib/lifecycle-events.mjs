...
export async function lifecycleSnapshot(){const gate=await privateStoreGate();if(!gate.ok)return{ok:false,error:gate.error};const [events,purchases,leads]=await Promise.all([
  ...
  return{ok:true,counts:{leads:leads.filter(i=>!i.pull_request).length,purchases:purchases.filter(i=>!i.pull_request).length,activations:uniq('learner_activated'),module_completions:parsed.filter(e=>e.event==='module_completed').length,course_completions:uniq('course_completed')},events:parsed};
}
