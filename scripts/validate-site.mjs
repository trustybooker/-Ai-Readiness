...
const corePublicPages=['index.html','booking.html','answers.html','checklist.html','checklist-start.html','lab.html','content-engine.html','courses.html','badge.html','refunds.html','privacy.html','purchase-success.html','thanks.html'];
const answerPages=fs.existsSync('answer')?fs.readdirSync('answer').filter(f=>f.endsWith('.html')).map(f=>`answer/${f}`):[];
const publicPages=[...corePublicPages,...answerPages];
for(const f of publicPages){if(!fs.existsSync(f))continue;const t=fs.readFileSync(f,'utf8');if(t.includes('fifynowllc.com/ai-readiness-pass'))failures.push(`${f} has legacy public URL`);if(t.includes('ai-readiness-pass.netlify.app'))failures.push(`${f} has off-domain public URL`);for(const phrase of ['lead tier for human follow-up','self-serve once tested','once enabled','payment record of truth','Human-value follow-up standard'])if(t.includes(phrase))failures.push(`${f} exposes internal language: ${phrase}`);if(/href\s*=\s*["']javascript:/i.test(t))failures.push(`${f} contains javascript: navigation`);}

function hrefs(html){return [...html.matchAll(/href\s*=\s*["']([^"']+)["']/gi)].map(m=>m[1].trim());}
function navHrefs(html){const nav=(html.match(/<nav\b[\s\S]*?<\/nav>/i)||[])[0]||'';return hrefs(nav);}
function pointsTo(list,target){return list.some(h=>{const x=h.split('?')[0].split('#')[0].replace(/^\.\//,'');return x===target||x===`../${target}`||x===`/${target}`||(target==='index.html'&&(x===''||x==='/'||x==='../index.html'));});}
const transactional=new Set(['purchase-success.html','thanks.html']);
for(const f of publicPages){if(!fs.existsSync(f)||f==='index.html')continue;const html=fs.readFileSync(f,'utf8'),nav=navHrefs(html);if(!nav.length){failures.push(`${f} missing primary navigation`);continue;}if(!pointsTo(nav,'index.html'))failures.push(`${f} navigation missing Home`);if(answerPages.includes(f)){if(!pointsTo(nav,'answers.html'))failures.push(`${f} navigation missing All answers`);if(!nav.some(h=>/assessment|checklist\.html|booking\.html/i.test(h)))failures.push(`${f} navigation missing Free score, Checklist, or Booking`);}else if(!transactional.has(f)){if(!nav.some(h=>/courses\.html|answers\.html/i.test(h)))failures.push(`${f} navigation missing Courses or Answers`);if(!nav.some(h=>/assessment|booking\.html/i.test(h)))failures.push(`${f} navigation missing Free score or Booking`);}}
