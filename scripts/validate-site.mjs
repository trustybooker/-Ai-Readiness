...
for (const file of answerPages) {
  if (!fs.existsSync(file)) continue;
  const text = fs.readFileSync(file, 'utf8');
  if (text.includes('| Fify Now LLC</title>')) failures.push(`${file} still uses legacy Fify Now LLC page title`);
  if (text.includes('<span>F</span>')) failures.push(`${file} still uses legacy F brand chip`);
  if (!text.includes('AI Kollege')) failures.push(`${file} missing AI Kollege brand`);
