...
for(const f of publicPages){if(!fs.existsSync(f)||f==='index.html')continue;const html=fs.readFileSync(f,'utf8'),nav=navHrefs(html);if(!nav.length){failures.push(`${f} missing primary navigation`);continue;}if(!pointsTo(nav,'index.html'))failures.push(`${f} navigation missing Home`);if(answerPages.includes(f)){if(!pointsTo(nav,'answers.html'))failures.push(`${f} navigation missing All answers`);if(!nav.some(h=>/assessment|checklist\.html/i.test(h)))failures.push(`${f} navigation missing Free score or Checklist`);}else if(!transactional.has(f)){if(!nav.some(h=>/courses\.html|answers\.html/i.test(h)))failures.push(`${f} navigation missing Courses or Answers`);if(!nav.some(h=>/assessment|booking\.html/i.test(h)))failures.push(`${f} navigation missing Free score or Booking`);}}
